...
export default function EarthquakeStructureActivity() {
  ...
  const startEarthquake = () => {
    ...
    const PATTERN = [2000, 1000, 1000, 1000, 1000]; // Vibrate for 1s, pause for 1s, repeat 5 times
  ...
  const handleComplete = async () => {
    // Correct: We check the first item in the array using
    if (designs.length === 0 || !designs[0].design.trim()) {
      alert("Please enter at least one design before completing.");
      return;
    }

    try {
      // Safely save the dynamic array locally to prevent router string limits
      await AsyncStorage.setItem(
        "@temp_earthquake_data",
